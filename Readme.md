...
ColdWallet 应用是一个基于 ESP32 开发的数字货币冷钱包应用，旨在提供安全可靠的数字货币管理服务。该项目的目标是实现一键转入功能，以提升用户的使用体验，同时保障用户资产的安全。

## 对标竞品

本项目对标竞品为 [Trezor Firmware](https://github.com/trezor/trezor-firmware) 和 Ledger，旨在提供类似的钱包管理功能，并通过 ESP32 设备提供更高的安全性。
...
- **设备发现与配对：** 使用蓝牙或其他无线技术进行设备的发现和配对。
- **安全验证：** 用户需通过输入密码进行安全握手，确保连接的安全性并验证设备的合法性。

## 实现步骤

1. **设备发现与配对：** 发现 esp32 并配对,然后在 React Native 应用上完成密码输入.
